...
def get_workspace_path(args=None):
...
def get_workspace_manifest_file(args=None):
    path = get_workspace_path(args)
    return os.path.join(os.path.join(path, "repo"), "Manifest.xml")

def get_workspace_manifest(args=None):
    manifest_path = get_workspace_manifest_file(args)
